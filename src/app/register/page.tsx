  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/register', {
        ...
        body: JSON.stringify({ username: username.trim(), password, displayName: displayName.trim(), deviceId }),
      });
    ...
    <div className="min-h-screen ambient-bg bg-[hsl(222,47%,6%)] flex items-center justify-center p-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-1">Join the AnimeCord community</p>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Display Name</label>
              <input
                type="text"
                required
                value={displayName}
          ...
          <div className="text-center mt-5">
            ...
            <Link href="/login" className="text-primary hover:text-violet-300 text-sm font-semibold transition-colors">
              Login here
            </Link>
          </div>
        </div>
